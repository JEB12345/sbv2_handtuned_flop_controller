...
% line of the loop variable. Each column is the direction as defined above.
...
if currentIndex
    ...
    % Solution: Just pick the first loop defined in the array for the
    % current face.
    display('Selecting an alternative suitable loop...');
    
    nextLoop = loopSequence(m_currentFace,1);
    display(['The new loop is ' num2str(nextLoop)]);
end
