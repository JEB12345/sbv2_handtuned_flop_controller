...
%% Ask user if caps are off

capsQ = 1;

while(capsQ)
    cmd = input('Are caps already off? y/n\n','s');
    switch lower(cmd)
        case 'y'
            capsQ = 0;
        ...
        case 'n'
            ...
            Cmd.effort = [];
            Group.send(Cmd);
            disp('Keeping current position. Please remove caps. Press ''y'' when done.');

        otherwise
            disp('Wrong command, please enter ''y'' or ''n''.');
    end
end
    ...
    while innerLoop
        ...
        switch lower(cmd)
            ...
            case 'kct' % keep current torque
                ...
                disp('Keeping current torque');
                innerLoop = 1;
                
            case 'z' % Go back to zero position
